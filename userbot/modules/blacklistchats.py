...
from userbot.core.data import blacklist_chats_list
...
@register(outgoing=True, pattern=r"^\.chatblacklist (on|off)$")
...
@register(outgoing=True, pattern=r"^\.addblkchat(s)?(?:\s|$)([\s\S]*)")
...
@register(outgoing=True, pattern=r"^\.delblkchat(s)?(?:\s|$)([\s\S]*)")
async def add_blacklist_chat(event):
...
@register(outgoing=True, pattern=r"^\.listblkchats$")
async def add_blacklist_chat(event):
